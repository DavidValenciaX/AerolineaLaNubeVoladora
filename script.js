document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("http://localhost/user-service/dashboard.php")
    .then((response) => {
    ...
    .then((data) => {
      if (data) {
        ...
        document.getElementById("role").innerText = `Rol: ${data.usuario_rol}`;

        const navbar = document.getElementById("navbar");
        let permisosHtml = "";

        // Convertir los valores de data.permisos a un arreglo
        const permisosArray = Object.values(data.permisos);
        ...
        document.getElementById("logoutBtn").addEventListener("click", () => {
          fetch("http://localhost/user-service/logout.php", { method: "POST" })
